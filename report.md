...
| Endpoint             | Acción                                           | Verbo HTTP | Sintaxis de llamada                                                          | Parámetros                                                                                 |                                       |                                                               |
| -------------------- | ------------------------------------------------ | ---------: | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ------------------------------------- | ------------------------------------------------------------- |
| `/symptoms`          | Listar síntomas por paciente (recientes primero) |        GET | `{BASE_URL}/symptoms?patientId={id}&_sort=timestamp&_order=desc&_limit={n}`  | Query: `patientId` (req), `_sort`, `_order`, `_limit` (opt)                                |                                       |                                                               |
| `/symptoms`          | Registrar nuevos síntomas                        |       POST | `{BASE_URL}/symptoms`                                                        | Body JSON: campos clínicos y `timestamp`                                                   |                                       |                                                               |
| `/symptoms/{id}`     | Editar/actualizar un registro de síntomas        |      PATCH | `{BASE_URL}/symptoms/{id}`                                                   | Path: `id` (req); Body parcial                                                             |                                       |                                                               |
| `/symptoms/{id}`     | Eliminar un registro de síntomas                 |     DELETE | `{BASE_URL}/symptoms/{id}`                                                   | Path: `id` (req)                                                                           |                                       |                                                               |
| `/medications`       | Listar medicación activa del paciente            |        GET | `{BASE_URL}/medications?patientId={id}&isActive=true`                        | Query: `patientId` (req), `isActive` (opt)                                                 |                                       |                                                               |
| `/medications`       | Crear prescripción (mock)                        |       POST | `{BASE_URL}/medications`                                                     | Body JSON: `patientId`, `name`, `dosage`, `frequency`, `schedule`, `prescribedBy`          |                                       |                                                               |
| `/adherence`         | Registrar toma/salto de dosis                    |       POST | `{BASE_URL}/adherence`                                                       | Body JSON: `medicationId`, `patientId`, `scheduledTime`, `takenAt`, `confirmed`, `skipped` |                                       |                                                               |
| `/adherence`         | Listar adherencia por medicamento/paciente       |        GET | `{BASE_URL}/adherence?patientId={id}&medicationId={id}`                      | Query: `patientId` (opt), `medicationId` (opt)                                             |                                       |                                                               |
| `/alerts`            | Listar alertas activas del paciente              |        GET | `{BASE_URL}/alerts?patientId={id}&resolved=false`                            | Query: `patientId` (req), `resolved` (opt)                                                 |                                       |                                                               |
| `/alerts/{id}`       | Marcar alerta como resuelta                      |      PATCH | `{BASE_URL}/alerts/{id}`                                                     | Path: `id` (req); Body: `{ "resolved": true }`                                             |                                       |                                                               |
| `/appointments`      | Listar citas del paciente/médico                 |        GET | `{BASE_URL}/appointments?patientId={id}` o `?doctorId={id}`                  | Query: `patientId` **o** `doctorId`                                                        |                                       |                                                               |
| `/appointments`      | Crear cita                                       |       POST | `{BASE_URL}/appointments`                                                    | Body JSON: `patientId`, `doctorId`, `date`, `time`, `type`, `status`                       |                                       |                                                               |
| `/appointments/{id}` | Actualizar estado de cita                        |      PATCH | `{BASE_URL}/appointments/{id}`                                               | Path: `id` (req); Body parcial                                                             |                                       |                                                               |
| `/diagnoses`         | Listar diagnósticos del paciente                 |        GET | `{BASE_URL}/diagnoses?patientId={id}`                                        | Query: `patientId` (req)                                                                   |                                       |                                                               |
| `/treatmentPlans`    | Ver plan(es) de tratamiento activos              |        GET | `{BASE_URL}/treatmentPlans?patientId={id}&status=active`                     | Query: `patientId` (req), `status` (opt)                                                   |                                       |                                                               |
| `/records`           | Listar registros clínicos (vitales/consultas)    |        GET | `{BASE_URL}/records?patientId={id}&type={vital_signs                         | symptoms                                                                                   | consultation}&_sort=date&_order=desc` | Query: `patientId` (req), `type`, `_sort`, `_order`, `_limit` |
| `/devices`           | Dispositivos vinculados al paciente              |        GET | `{BASE_URL}/devices?patientId={id}&isActive=true`                            | Query: `patientId` (req), `isActive` (opt)                                                 |                                       |                                                               |
| `/messages`          | Listar mensajes entre usuario A y B              |        GET | `{BASE_URL}/messages?senderId={id}&receiverId={id}`                          | Query: `senderId`, `receiverId`                                                            |                                       |                                                               |
| `/messages`          | Enviar mensaje                                   |       POST | `{BASE_URL}/messages`                                                        | Body JSON: `senderId`, `receiverId`, `subject`, `content`, `timestamp`                     |                                       |                                                               |
| `/patients/{id}`     | Obtener perfil del paciente                      |        GET | `{BASE_URL}/patients/{id}`                                                   | Path: `id` (req)                                                                           |                                       |                                                               |
| `/doctors`           | Listar doctores (por tenant o independientes)    |        GET | `{BASE_URL}/doctors?tenantId={id}&isIndependent={bool}`                      | Query: `tenantId` (opt), `isIndependent` (opt)                                             |                                       |                                                               |
| `/subscriptions`     | Ver suscripción del paciente/tenant              |        GET | `{BASE_URL}/subscriptions?patientId={id}` o `?payerType=tenant&payerId={id}` | Query según caso                                                                           |                                       |                                                               |
| `/nudges`            | Listar recordatorios/notificaciones              |        GET | `{BASE_URL}/nudges?patientId={id}&isDismissed=false`                         | Query: `patientId` (req), `isDismissed`/`isSnoozed` (opt)                                  |                                       |                                                               |
| `/users`             | Buscar usuario por email o rol                   |        GET | `{BASE_URL}/users?email={mail}` o `?role={patient                            | doctor                                                                                     | hospital_admin}`                      | Query: `email`/`role`                                         |
| `/tenants`           | Ver instituciones (mock)                         |        GET | `{BASE_URL}/tenants?status=active`                                           | Query: `status` (opt)                                                                      |                                       |                                                               |
| `/staff`             | Listar personal del tenant                       |        GET | `{BASE_URL}/staff?tenantId={id}`                                             | Query: `tenantId` (req)                                                                    |                                       |                                                               |
